...
```bash
get-weather$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
get-weather$ sam local start-api
get-weather$ curl http://localhost:3000/
```
...
```bash
get-weather$ sam logs -n HelloWorldFunction --stack-name get-weather --tail
```
...
```bash
sam delete --stack-name get-weather
```
